import os
import logging
from fastapi import FastAPI, Query  # 🔥 أضفنا FastAPI هنا
from fastapi.middleware.cors import CORSMiddleware

logger = logging.getLogger(__name__)

# 🔥 إنشاء كائن الـ FastAPI هنا مباشرة لكسر الحلقة الدائرية
web_app = FastAPI()
...
# مرجع ديناميكي لكائن الـ Application الخاص بـ تيليجرام لتبادل البيانات
...
# ===============================================
# مسارات الـ API والواجهة المكتبية الذكية (تبقى كما هي تماماً...)
# ===============================================
@web_app.get("/miniapp")
async def get_miniapp():
    ...
    return fastapi.responses.HTMLResponse(content="<h3>عذراً، لم يتم العثور على ملف الواجهة index.html</h3>", status_code=404)

@web_app.get("/api/search")
async def api_search(query: str = Query(..., min_length=2)):
    if bot_application is None:
        return {"success": False, "message": "السيرفر قيد التهيئة..."}
