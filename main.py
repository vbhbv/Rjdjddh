...
async def start(update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not await check_subscription(update.effective_user.id, context.bot):
        ...
        await update.message.reply_text("يرجى الاشتراك أولاً", reply_markup=keyboard)
        return

    await update.message.reply_text("📚 اكتب اسم الكتاب للبحث")
...
async def search_books_with_subscription(update, context: ContextTypes.DEFAULT_TYPE):
    if not await check_subscription(update.effective_user.id, context.bot):
        await update.message.reply_text(f"🚫 يجب الاشتراك في {CHANNEL_USERNAME}")
        return
