...
CHANNEL_USERNAME = os.getenv("CHANNEL_USERNAME")  # القناة المطلوبة
ADMIN_ID = int(os.getenv("ADMIN_ID", "0"))        # آي دي الآدمن لإرسال الاشعارات
...
async def ensure_subscription(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if member.status in ["member", "creator", "administrator"]:
            # سجل المستخدم إذا لم يكن موجوداً
                ...
                if not exists:
                    ...
                    if ADMIN_ID:
                        await context.bot.send_message(
                            chat_id=ADMIN_ID,
                            text=f"اشترك شخص في قناتك @{CHANNEL_USERNAME}"
                        )
            return True
        else:
            await update.message.reply_text(f"❌ يرجى الاشتراك أولاً في القناة: @{CHANNEL_USERNAME}")
            return False
    except Exception:
        await update.message.reply_text(f"❌ يرجى الاشتراك أولاً في القناة: @{CHANNEL_USERNAME}")
        return False
...
async def search_books_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    allowed = await ensure_subscription(update, context)
    if not allowed:
        return

    query = update.message.text.strip()
    if not query:
        await update.message.reply_text("📖 أرسل اسم الكتاب للبحث.")
        return
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text(
        ...
        "ابحث عن أي كتاب مباشرة بإرسال اسمه.",
        parse_mode="Markdown"
    )
